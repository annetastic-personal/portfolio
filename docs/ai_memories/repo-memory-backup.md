# Portfolio repo memory backup

This file mirrors the contents of the /memories/repo/ files for the portfolio repo, kept in-repo for durability.
...
- Styling/components: prefer Tailwind CSS utilities and daisyUI components over custom CSS or hand-built components.
- If there's a strong case for custom styling/components instead, explain the case and get confirmation before implementing.

---

## portfolio-wiki-publishing-plan.md

# portfolio wiki publishing plan
...
3. After pulling, run git diff --name-only <old-HEAD>..<new-HEAD> to get the list of files that changed as a result of the pull (i.e., pages edited directly online/via web UI since the last publish run).
